...
class AcroEnvNew(core.Env):
    ...
    MOI = 0.00100  #: moments of inertia for both links
    ...
    def step(self, act):
        ...
        #reward = 0.
        #if ( (s_new[0]>= (pi/2.0)-self.DELTA) and (s_new[0]<= (pi/2.0) +self.DELTA) ): 
        #    if( (s_new[1]<=self.DELTA) and (s_new[1]>= -self.DELTA) ): 
        #        reward = 1.
        done = bool(-np.cos(s_new[0]) - np.cos(s_new[1] + s_new[0]) > 1.) #self._terminal()
