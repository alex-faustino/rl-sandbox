...
class qLearningNetwork(object):
 def __init__(self,env):
  ...
  if self.states.shape[0] == 1:
   self.N, self.D_in, self.H, self.D_out = 54, 1, 90, self.allowed_actions.shape[0]
  else:
   self.N, self.D_in, self.H, self.D_out = 1, self.states.shape[1], 90, self.allowed_actions.shape[0]
