...
class SARSA():
    ...
    def update_q(self, state_old, action_old, reward, state_new, action_new, alpha):
        new_state_index = self.hash_code(state_new)
        old_state_index = self.hash_code(state_old)
        self.Q[old_state_index][action_old] = self.Q[old_state_index][action_old] + alpha * (reward + self.gamma * self.Q[new_state_index][action_new] - self.Q[old_state_index][action_old])
    ...
    def run(self):
        ...
        for e in range(self.n_episodes):
            ...
            for k in range(self.n_steps):
                ...
                new_state = obs
                new_action = self.take_action(new_state, epsilon)
                self.update_q(current_state, action, reward, new_state, new_action, alpha)
            ...
            if e == self.n_episodes-1:
                print('This is SARSA \'', self.action_mode, '\' version')
