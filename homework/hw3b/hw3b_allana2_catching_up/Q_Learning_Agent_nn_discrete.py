...
class qLearning(object):
    def __init__(self, env, my_nn, my_nn2, render_input): 
        ...
        self.num_allowed_actions = env.action_space.n

        self.env.reset()#random initialize

#        self.env.action_space.high # use for continuous actions
#        self.env.action_space.low  # use for continuous actions

        self.action = np.random.randint(0,self.env.action_space.n) # use for discrete actions
        ...
        self.my_gamma = 0.9
        self.my_epsilon = 0.1
        self.my_q_function = np.random.rand(1,self.env.action_space.n)# randomly initialized placeholder for q_function for scalar state space
        ...
        self.replay_length = 10000#250 is edge of reasonable for run-time, let's see how this affects results
        self.minibatch_size = my_nn.reportMinibatchSize()#using same minibatch size for my_nn and my_nn2
        self.minibatch_log = np.random.rand(self.env.observation_space.shape[0],self.minibatch_size)# state,action,reward, next state
