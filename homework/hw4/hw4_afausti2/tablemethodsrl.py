...
class Reinforce(GenTableMethodsRL):
    ...
    def get_trajectory(self, episode_length, policy):
        ...
        for t in range(episode_length):
            traj_a[t] = self.get_action(traj_s[t], policy)
    ...
    def train(self, episodes_num, batch_size, episode_length):
        ...
        for batch in range(batches_num):
            ...
            for n in range(batch_size):
                ...
                s, a, r = self.get_trajectory(episode_length, policy)

                J = np.zeros(policy.shape)
                reward = 0
                for t in range(episode_length):
                    # update the gradient
                    J += self.get_grad(s[t], a[t], policy)
                    reward += r[t]
