...
class Featurizer():
    ...
    def __init__(self, env):
        ...
        observation_samples = np.array([env.observation_space.sample() \
                                        for _ in range(1000)]).reshape(-1, 1).astype(np.float64)
        self.scaler = sklearn.preprocessing.StandardScaler()
        self.scaler.fit(observation_samples)

        # Convert a state to a featurized representation.
        ...
        self.featurizer = sklearn.pipeline.FeatureUnion([
                    ("rbf1", RBFSampler(gamma=100.0, n_components=n_components)),
                    ("rbf2", RBFSampler(gamma=10.0, n_components=n_components)),
                    ("rbf3", RBFSampler(gamma=0.1, n_components=n_components)),
                    ("rbf4", RBFSampler(gamma=0.01, n_components=n_components))
                    ], n_jobs=-1)
        self.featurizer.fit(self.scaler.transform(observation_samples))
        self.n_features = len(self.featurizer.transformer_list) * n_components

    def feature_state(self, state):
        """
        Returns the featurized representation of a state.
        ...
        """
        state = np.reshape(state, (-1, 1)).astype(np.float64)  # ensures 2-D array
        scaled = self.scaler.transform(state)
        featurized = self.featurizer.transform(scaled)
        #print(featurized.flatten())
        
        return 20*featurized.flatten()
'''    
class Policy():
    """
...
class run_REPS(object):    
    def REPS(self, env, R, policy, features, x0, epsilon = 0.9, info_loss=1,
        ...
        for i in range(n_policy_updates):
            ...
            J[i] = expected_return(env, R, policy, 50, 100)
