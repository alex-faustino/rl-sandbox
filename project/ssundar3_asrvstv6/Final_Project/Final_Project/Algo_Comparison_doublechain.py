...
import gym_env
import gym
from RL_Agents import sarsa, Risca, double_chain_REPS_class_version, Reinforce


sarsa = importlib.reload(sarsa)
Risca = importlib.reload(Risca)
double_chain_class_version = importlib.reload(double_chain_REPS_class_version)
Reinforce = importlib.reload(Reinforce)
num_run = 5

#Creating object of Sarsa class and passing environment
env = gym.make('double_chain-v0')
...
features = [[1, 0, 0], [1, 1, 1], [1, 2, 4], [1, 3, 9], [1, 4, 16], [1, 5, 25], [1, 6, 36], [1, 7, 49], [1, 8, 64]]
...
R = np.array([[0, 0], [0, 2], [0, 2], [0, 2], [10, 2], [0, 2], [0, 2], [0, 2], [5, 2]])
info_loss=0.01
n_policy_updates=30
n_samples= 500
opt_method='L-BFGS-B'
...
for item in range(num_run):
    ...
    rewards_risca = []
    rewards_reps = []
    policy = double_chain_class_version.Policy_REPS(env)
    agent = double_chain_class_version.run_REPS()
