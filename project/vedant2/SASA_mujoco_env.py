...
class SASAmujocoEnv:
    ...
    def __init__(self,Noise = None,params = None):
        ...
        self.Params = params
        self.model = load_model_from_path(os.path.join(os.getcwd(),'SASA.xml'))
        ...
        self.dt = 1e-4
        self.njoints = 1
        self.action_dim = self.njoints*2*3;
    ...
    def step(self,a,final = False, render = False):
        ...
        if (self.Noise != None):
            for x in self.Noise['Freq']:
                action_noise[0] =  math.cos(self.t *2*np.pi* self.Fx+self.phaseXL) * self.Dx#math.cos(t / 10.) * 
                action_noise[1] =  math.cos(self.t *2*np.pi* self.Fx+self.phaseXL) * self.Dy
                action_noise[2] =  math.cos(self.t *2*np.pi* self.Fx+self.phaseXL) * self.Dz
                
                action_noise[3] =  math.cos(self.t *2*np.pi* self.Fx+self.phaseXL)* self.Rx #math.cos(t / 10.) * 
    ...
    def set_init(self, tough = False):
        q_init = np.array([0,0,0,1])
        q_init = q_init/np.linalg.norm(q_init)
        w_init = np.array([0,0,0])
        #w_init = (w_init/np.linalg.norm(w_init))
        q_list_init = np.zeros_like(self.sim.data.qpos)
        q_list_init[0:4]  = q_init
        w_list_init = np.zeros_like(self.sim.data.qvel)
        w_list_init[0:3]  = w_init
        self.sim.data.qpos[:] = q_list_init;
        self.sim.data.qvel[:] = w_list_init;
        self.sim.forward()
        self.x = np.concatenate((q_list_init,w_init)) 
    ...
    def get_obs(self):
        ...
        w = self.sim.data.qvel[0:3]
        self.x = np.concatenate((q,w))
        return self.x
    def get_reward(self,obs,action):
        
        reward = ((np.arccos(obs[3])/(np.pi/2)))#np.exp/np.e-(np.linalg(a)/(2000000*np.sqrt(6)))
        return reward
    def copy(self):
        c = SASAmujocoEnv()
        c.s = self.x.copy()
