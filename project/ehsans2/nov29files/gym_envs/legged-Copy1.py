...
class robotBulletEnv(gym.Env):
    ...
    def get_state(self, robot, physicsClientId):
        ...
        state = joint_angles + joint_velocities + link_poses +  link_ornts + link_lin_vel + link_ang_vel 
    ...
    def reset(self):
        if self.init_state_id is None:
            ...
            self.robot = p.loadURDF(self.robot_urdf, basePosition = [0, 0, 1.02], 
            ...
            for bodyunqid in [self.plane, self.robot]:
                ...
                self.change_friction_coeffs(bodyUniqueId = bodyunqid, physicsClientId = self.physicsClientId, 
                                            links_idx_list='all', lateral_coeff_list = 1., spinning_friction_list = 0.,
                                            rolling_friction_list = 0.)
        ...
        deactivate_joint_motors(self.robot, physicsClientId = self.physicsClientId)
        
        real_time = True if self.gui else False
        ...
        p.setRealTimeSimulation(real_time, physicsClientId = self.physicsClientId)
