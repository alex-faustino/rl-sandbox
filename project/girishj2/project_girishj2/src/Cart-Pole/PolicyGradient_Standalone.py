...
env.env.set_prop(9.8,1.0,0.1,0.75,10)
    ...
    for epoch in range(MAX_EPISODE):
        ...
        for ep_length in range(LEN_EPISODE):
            ...
            if done:
                ...
                break
        if epoch % 100 == 0:
            Avg_Reward.append(np.mean(total_reward[-100:]))
            print('Epoch: %i' %epoch, 'Avg Reward: %i'%np.mean(total_reward[-100:]))

    with open(file_name,'wb') as file:
        pickle.dump(Avg_Reward, file)
