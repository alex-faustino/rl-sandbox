...
file_name = 'PPO_Pendulum/pendStandalone_Results'
...
class policy(object):
    ...
    def policy_net(self, name ,train_status):
        ...
        with tf.variable_scope(name): 
            ...
            #actor_prob = tf.contrib.distributions.Normal(mu=mu, sigma=sigma)
...
class PPO:
    ...
    def save_net(self):
        file_name = 'PPO_Pendulum/pendStandalone_Params'
        data = self.sess.run(self.new_policy.network_param)
    ...
    def add_to_buffer(self, state, action, reward, done,next_state):
        self.replay_buffer.add(np.reshape(state,(self.s_dim,)),np.reshape(action,(self.a_dim,)),reward,done,np.reshape(next_state,(self.s_dim,)))
        

def main():
    env = gym.make('Pendulum-v2')
    env.env.set_property(10,1.5,2.5, True)
    env.seed(0)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(MAX_EPOCH):
            ...
            for step in range(MAX_EP_LEN):
                ...
                agent.add_to_buffer(s,action,r,done,s1)
                running_r += r
                #env.render()
                s = s1
                if (step+1) % mini_batch_size == 0 or step == MAX_EP_LEN-1:

                    s_batch,a_batch,r_batch,t_batch,s2_batch =  agent.replay_buffer.sample_batch(mini_batch_size)

                    r_batch = (r_batch- np.mean(r_batch))/(np.var(r_batch) + 0.001)
